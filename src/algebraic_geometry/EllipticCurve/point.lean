...
{ to_fun    := point_hom.to_fun φ,
  ...
  begin
    rintro (_ | _) (_ | _),
    any_goals { refl },
      by_cases h_x: x_x ≠ y_x,
      { change point_hom.to_fun φ (EllipticCurve.add (some x_x x_y x_w) (some y_x y_y y_w))
      = EllipticCurve.add (point_hom.to_fun φ (some x_x x_y x_w)) (point_hom.to_fun φ (some y_x y_y y_w)),
        have hφ_x: φ(x_x) ≠ φ(y_x),
        {begin intro h,
        exact h_x (φ.to_ring_hom.injective h), end},
        rw [EllipticCurve.add, dif_pos $ h_x],
        unfold point_hom.to_fun,
        rw [EllipticCurve.add , dif_pos $ hφ_x],
        simp only,
        split,
        simp,
        simp only [map_add,map_pow,map_mul,map_sub,alg_hom.map_inv,alg_hom.commutes, map_neg],
      },
      { have hφ_x: φ(x_x) = φ(y_x),
        {begin congr,
        exact not_not.mp h_x, end},
        have: φ(x_y) + φ(y_y) + (F↑L)E.a₁ * φ(y_x) + (F↑L)E.a₃  = φ(x_y + y_y + (F↑K)E.a₁ * y_x + (F↑K)E.a₃),
        {simp only [map_add, map_mul, alg_hom.commutes],},

        by_cases h_y: x_y + y_y + (F↑K)E.a₁ * y_x + (F↑K)E.a₃ = 0,
        { have  hφ_y: φ(x_y) + φ(y_y) + (F↑L)E.a₁ * φ(y_x) + (F↑L)E.a₃ = 0,
          {by rw [this,h_y,map_zero],},
          unfold point_hom.to_fun,
          simp [not_not.mp h_x, hφ_x, h_y,hφ_y],
          unfold point_hom.to_fun,
        },
        {
          have hφ_y: φ(x_y) + φ(y_y) + (F↑L)E.a₁ * φ(y_x) + (F↑L)E.a₃ ≠ 0,
          {begin rw [this],intro h, rw <- map_zero φ at h, exact h_y (φ.to_ring_hom.injective h), end},
          unfold point_hom.to_fun,
          change point_hom.to_fun φ (EllipticCurve.add (some x_x x_y x_w)(some y_x y_y y_w)) = EllipticCurve.add (some (φ x_x) (φ x_y) _) (some (φ y_x) (φ y_y) _),
          --unfold has_add.add,
          rw [EllipticCurve.add, dif_neg $ h_x, if_pos h_y,EllipticCurve.dbl],
          rw [EllipticCurve.add, dif_neg $ not_not.mpr hφ_x, if_pos hφ_y,EllipticCurve.dbl],
          have: φ (2 * x_y + (F↑K) E.a₁ * x_x + (F↑K) E.a₃) = 2 * φ x_y + (F↑L) E.a₁ * φ x_x + (F↑L) E.a₃,
          {by simp only [map_add, map_mul, map_bit0, map_one, alg_hom.commutes],},
          by_cases h_y : 2 * x_y + (F↑K) E.a₁ * x_x + (F↑K) E.a₃ ≠ 0,
          {
            have hφ_y: 2 * φ x_y + (F↑L) E.a₁ * φ x_x + (F↑L) E.a₃ ≠ 0,
            {begin rw <- this, intro h, rw <- map_zero φ at h, exact h_y (φ.to_ring_hom.injective h), end},
            rw [dif_pos $ h_y, dif_pos $ hφ_y],
            unfold point_hom.to_fun,
            simp only [map_add,map_pow,map_mul,map_sub,alg_hom.map_inv,alg_hom.commutes, map_neg, map_bit0,map_bit1, map_one],
            split,
            any_goals {refl},
          },
          {
            have hφ_y: 2 * φ x_y + (F↑L) E.a₁ * φ x_x + (F↑L) E.a₃ = 0,
            {by rw [<- this, not_not.mp h_y, map_zero φ],},
            rw [dif_neg $ h_y, dif_neg $ not_not.mpr hφ_y],
            unfold point_hom.to_fun,
          },
          --simp only [map_add,map_pow,map_mul,map_sub,alg_hom.map_inv,alg_hom.commutes, map_neg],
          --simp [not_not.mp h_x, h_y],
          --simp [eq.refl (φ y_x), hφ_y],
      ...
      --unfold point_hom.to_fun,
    --unfold EllipticCurve.add point_hom.to_fun,
    --by_cases x_x_ne_y_x: x_x ≠ y_x,
    --{
      --simp[x_x_ne_y_x,x_x_ne_y_x],
    --}
...
{ smul      := point_gal,
  ...
  begin
    ...
    by_cases h_x: x_x ≠ y_x,
    {
      simp [h_x],
      unfold point_gal,
      have hσ_x: σ • x_x ≠ σ • y_x,
      {
        begin intro h, exact h_x (σ.injective h) end
      },
      rw [EllipticCurve.some_add_some_of_x_ne],
      simp [alg_equiv.commutes],
      split,
      refl,
    },
  end,
